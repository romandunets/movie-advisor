...
public class FollowerToFollowedDaoImpl implements FollowerToFollowedDao {
    ...
    public List<FollowerToFollowed> getFollowerList(long id) throws Exception {
        session = sessionFactory.openSession();
        tx = session.beginTransaction();
        List<FollowerToFollowed> results = session.createSQLQuery(
                "SELECT * FROM follower_to_followed " +
                "where follower_to_followed.follower_id=:id")
    ...
    public List<FollowerToFollowed> getFollowedList(long id) throws Exception {
        session = sessionFactory.openSession();
        tx = session.beginTransaction();
        List<FollowerToFollowed> results = session.createSQLQuery(
                "SELECT * FROM follower_to_followed " +
                "where follower_to_followed.followed_id=:id")
                .addEntity(FollowerToFollowed.class)
                .setParameter("id", id).list();
